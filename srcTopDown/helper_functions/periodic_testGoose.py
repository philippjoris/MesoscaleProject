...
def nodesPeriodic3D(mesh):
    ...
    fro = mesh.nodesFrontFace
    bck = mesh.nodesBackFace
    lft = mesh.nodesLeftFace
    rgt = mesh.nodesRightFace
    bot = mesh.nodesBottomFace
    top = mesh.nodesTopFace
    # Open Edge Nodes (excluding corners)
    froBot = mesh.nodesFrontBottomOpenEdge
    froTop = mesh.nodesFrontTopOpenEdge
    froLft = mesh.nodesFrontLeftOpenEdge
    froRgt = mesh.nodesFrontRightOpenEdge
    bckBot = mesh.nodesBackBottomOpenEdge
    bckTop = mesh.nodesBackTopOpenEdge
    bckLft = mesh.nodesBackLeftOpenEdge
    bckRgt = mesh.nodesBackRightOpenEdge
    botLft = mesh.nodesBottomLeftOpenEdge
    botRgt = mesh.nodesBottomRightOpenEdge
    topLft = mesh.nodesTopLeftOpenEdge
    topRgt = mesh.nodesTopRightOpenEdge
    # Corner Nodes (single indices)
    corner_FBL = mesh.nodesFrontBottomLeftCorner
    corner_FBR = mesh.nodesFrontBottomRightCorner
    corner_BBR = mesh.nodesBackBottomRightCorner
    corner_BBL = mesh.nodesBackBottomLeftCorner
    corner_FTL = mesh.nodesFrontTopLeftCorner
    corner_FTR = mesh.nodesFrontTopRightCorner
    corner_BTR = mesh.nodesBackTopRightCorner
    corner_BTL = mesh.nodesBackTopLeftCorner
