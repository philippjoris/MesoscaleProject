...
def parse_msh(msh_filepath, cohesive_line_names=["CohesiveParticle", "CohesiveInterface"], tolerance=1e-3):
    """
    ...
    intersection_original_side_nodes = {name: {} for name in cohesive_line_names}
    intersection_node_dublicates = {name: {} for name in cohesive_line_names} 
    ...
    for original_gmsh_node_id in range(1, num_original_nodes + 1):
        ...
        # Re-dublicating node on cohesive Particle/Interface intersection. To have four nodes of the original node later on.
        if is_on_cohesive_particle and is_on_cohesive_interface:            
            # This is the "original" node of the particle side of the intersection. As this node is already a duplicate from the particle surface, the original
            # node of the intersection is considered to be the original particle dublicate.   

            # This will be the node belonging to the lower particle bulk later.
            intersection_original_side_nodes["CohesiveParticle"][original_gmsh_node_id] = interface_node_duplicates["CohesiveParticle"][original_gmsh_node_id]  
            
            # This will be the node belonging to the lower matrix bulk later.
            intersection_original_side_nodes["CohesiveInterface"][original_gmsh_node_id] = original_gmsh_node_id

            # This will be the node belonging to the upper matrix bulk later.
            intersection_node_dublicates["CohesiveInterface"][original_gmsh_node_id] = interface_node_duplicates["CohesiveInterface"][original_gmsh_node_id]

            # store a duplicated node of the node on cohesiveParticle (this is already a dublicate!).
            # This will be the node belonging to the upper particle bulk later.
            if original_gmsh_node_id not in intersection_node_dublicates["CohesiveParticle"]:
                intersection_node_dublicates["CohesiveParticle"][original_gmsh_node_id] = next_available_node_id
                new_coords_list.append(coor[original_gmsh_node_id - 1])
                next_available_node_id += 1              

    # Notice: If a node is both on Cohesive Particle and Cohesive Interface, the node is dublicated three times!
    ...
    for interface_name in cohesive_line_names:
        ...
        if interface_name == "CohesiveParticle":            
            ...
            node_ids = np.array(sorted(particle_line_node_ids)) - 1
            coords = coor[node_ids]
            particle_center_x, particle_center_y = coords.mean(axis=0)
            particle_xmin = coords[:, 0].min()
            particle_xmax = coords[:, 0].max()
            particle_radius = 0.5 * (particle_xmax - particle_xmin)

            num_particle_nodes = len(particle_line_node_ids)
            
            if num_particle_nodes > 0:
                particle_center = np.array([particle_center_x, particle_center_y])
                all_particle_centers.append([particle_center, particle_radius]) 
            else:
            ...
            for contour_elements in all_ordered_particle_contours:
                for line_elem in contour_elements:
                    ...
                    n1check = intersection_node_dublicates["CohesiveParticle"].get(actual_n1_gmsh_orig)
                    n2check = intersection_node_dublicates["CohesiveParticle"].get(actual_n2_gmsh_orig)
                    ...
                    if n2check is not None and elem_midpoint[1] > 0:
                        id_bl = interface_node_duplicates["CohesiveInterface"][actual_n2_gmsh_orig]
                        id_tl = intersection_node_dublicates["CohesiveParticle"][actual_n2_gmsh_orig]
                    else:
                    ...
                    if n1check is not None and elem_midpoint[1] > 0:
                        id_br = interface_node_duplicates["CohesiveInterface"][actual_n1_gmsh_orig]
                        id_tr = intersection_node_dublicates["CohesiveParticle"][actual_n1_gmsh_orig]
                    else:    
        ...
        elif interface_name == "CohesiveInterface":
            ...
            for line_elem in cohesive_line_elements_by_name[interface_name]:
                ...
                n1_line_tags = node_to_physical_line_tags.get(n1_gmsh_orig, set())
                n2_line_tags = node_to_physical_line_tags.get(n2_gmsh_orig, set())
                n1_on_cohesive_interface = cohesive_interface_tag is not None and cohesive_interface_tag in n1_line_tags
                n1_on_cohesive_particle = cohesive_particle_tag is not None and cohesive_particle_tag in n1_line_tags
                n2_on_cohesive_interface = cohesive_interface_tag is not None and cohesive_interface_tag in n2_line_tags
                n2_on_cohesive_particle = cohesive_particle_tag is not None and cohesive_particle_tag in n2_line_tags     

                # Calculate midpoint of this line element (or just its average node coord)
                mid_x = (coor[n1_gmsh_orig - 1, 0] + coor[n2_gmsh_orig - 1, 0]) / 2.0
                mid_y = (coor[n1_gmsh_orig - 1, 1] + coor[n2_gmsh_orig - 1, 1]) / 2.0
                elem_midpoint = np.array([mid_x, mid_y])           
                ...
                inside_particle = False
                for particle_midpoint in all_particle_centers:                    
                    # particle_midpoint[0] = particle_center
                    # particle_midpoint[1] = particle_radius
                    if np.linalg.norm(elem_midpoint - particle_midpoint[0]) < particle_midpoint[1]:
                        inside_particle = True

                if n1_on_cohesive_interface and n1_on_cohesive_particle and not inside_particle:
                    id_bl = intersection_original_side_nodes["CohesiveInterface"][n1_gmsh_orig]
                    id_tl = intersection_node_dublicates["CohesiveParticle"][n1_gmsh_orig]
                elif n1_on_cohesive_interface and n1_on_cohesive_particle and inside_particle:
                    id_bl = intersection_original_side_nodes["CohesiveParticle"][n1_gmsh_orig]
                    id_tl = intersection_node_dublicates["CohesiveInterface"][n1_gmsh_orig]            
                elif n2_on_cohesive_interface and n2_on_cohesive_particle and not inside_particle:
                    id_br = intersection_original_side_nodes["CohesiveInterface"][n2_gmsh_orig]
                    id_tr = intersection_node_dublicates["CohesiveParticle"][n2_gmsh_orig]
                elif n2_on_cohesive_interface and n2_on_cohesive_particle and inside_particle:
                    id_br = intersection_original_side_nodes["CohesiveParticle"][n2_gmsh_orig]
                    id_tr = intersection_node_dublicates["CohesiveInterface"][n2_gmsh_orig]          
    ...
    upper_particle_domain_tag = next( (tag for tag, name in physical_names.items() if name == "UpperParticleDomain"), None)
    lower_particle_domain_tag = next( (tag for tag, name in physical_names.items() if name == "LowerParticleDomain"), None)
    ...
    for elem_data in elements_raw: # Iterate through original parsed elements
        if elem_data['type'] == 3: # This is a 4-node quad (bulk element)
            ...
            for i, original_gmsh_node_id in enumerate(original_gmsh_node_ids):
                ...
                if bulk_phys_tag == upper_domain_tag:
                    ...
                    if cohesive_particle_tag is not None and cohesive_interface_tag is not None and \
                        ...
                        if original_gmsh_node_id in interface_node_duplicates['CohesiveInterface']:
                            mapped_node_id = intersection_node_dublicates["CohesiveParticle"][original_gmsh_node_id]   
                ...
                # Case 3: Bulk element is UpperParticleDomain
                elif bulk_phys_tag == upper_particle_domain_tag:
                    # ParticleDomain always connects to the duplicated (particle) side of CohesiveParticle 
                    if cohesive_particle_tag is not None and cohesive_interface_tag is not None and \
                        cohesive_particle_tag in node_line_tags and cohesive_interface_tag in node_line_tags:
                        if original_gmsh_node_id in interface_node_duplicates["CohesiveInterface"]:
                            mapped_node_id = interface_node_duplicates["CohesiveInterface"][original_gmsh_node_id]
                    elif cohesive_particle_tag is not None and cohesive_interface_tag is not None and cohesive_particle_tag in node_line_tags:
                    ...
                    elif cohesive_particle_tag  is not None and cohesive_interface_tag is not None and cohesive_interface_tag in node_line_tags:
                        if original_gmsh_node_id in interface_node_duplicates['CohesiveInterface']:
                           mapped_node_id = interface_node_duplicates['CohesiveInterface'][original_gmsh_node_id]                                  

                # Case 4: Bulk element is LowerParticleDomain            
                elif bulk_phys_tag == lower_particle_domain_tag:
                    # ParticleDomain always connects to the duplicated (particle) side of CohesiveParticle 
                    if cohesive_particle_tag is not None and cohesive_interface_tag is not None and \
                        cohesive_particle_tag in node_line_tags and cohesive_interface_tag in node_line_tags:
                        if original_gmsh_node_id in interface_node_duplicates["CohesiveParticle"]:
                        ...
                        if original_gmsh_node_id in interface_node_duplicates['CohesiveParticle']:
                           mapped_node_id = interface_node_duplicates['CohesiveParticle'][original_gmsh_node_id]
                    elif cohesive_particle_tag is None and cohesive_interface_tag is not None and cohesive_interface_tag in node_line_tags:
                        if original_gmsh_node_id in interface_original_side_nodes['CohesiveInterface']:
                           mapped_node_id = interface_original_side_nodes['CohesiveInterface'][original_gmsh_node_id]                                                    
                
                updated_node_ids[i] = mapped_node_id

            if (bulk_phys_tag == lower_particle_domain_tag) or (bulk_phys_tag == upper_particle_domain_tag):
                original_particle_line_nodes_set.update(updated_node_ids)
...
def main():
    mesh_file_name = "i.geo_simple_frag.msh2"
