...
from material_models import ElasticPK2E
import GooseFEM
import numpy as np
# mesh
# ----
...
# material definition
# -------------------
mat = ElasticPK2E(K=np.ones([nelem, nip]), G=np.ones([nelem, nip]))
# simulation variables
# --------------------
...
fe = elem.Int_gradN_dot_tensor2_dV(mat.PK2)
...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    for iter in range(max_iter): 
        ...
        P = np.matmul(F_inv, tau)
  
        # update internal forces and assemble
        fe = elem.Int_gradN_dot_tensor2_dV(P)
        fint = vector.AssembleNode(fe)

        # residual
        fres = -vector.AsDofs_u(fint)
        
        res_norm = np.linalg.norm(fres) / res_norm_zero
        if res_norm < 1e-06:
