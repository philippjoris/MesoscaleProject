...
meshRefined = GooseFEM.Mesh.Quad4.Map.RefineRegular(mesh, 2, 2)
...
if args.plot:
    ...
    print(meshRefined.coarseMesh.elementgrid)
    print(meshRefined.fineMesh.elementgrid)
    # meshRefined.averageToCoarse(vector.AsElement(coor + disp))

    
    fig, ax = plt.subplots(figsize=(8, 6))
    gplt.patch(coor=coor + disp, conn=conn, cindex=meshRefined.mapToFine(sigCoarsePlot), cmap="jet", axis=ax)
    ...
    mappable.set_array(sigCoarsePlot)
    ...
    # plot strain
    fig, ax = plt.subplots(figsize=(8, 6))
    gplt.patch(coor=coor + disp, conn=conn, cindex=meshRefined.mapToFine(epsCoarsePlot), cmap="jet", axis=ax)
    # gplt.patch(coor=coor, conn=conn, linestyle="--", axis=ax)
    ax.set_xlim(-7,90)
    # ax.set_ylim(0,110)
    ...
    # Add colorbar
    mappable = ScalarMappable(norm=plt.Normalize(), cmap=plt.colormaps["jet"])
    mappable.set_array(epsCoarsePlot)
