...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    disp[mesh.nodesRight, 0] += (+1.0/ninc)
    ...
    if not converged:
        print (f"Increment {ilam}/{ninc} not converged at Iter {iter}, Residual = {res_norm}")
...
# residual
fres = -vector.AsDofs_u(fint)
print(np.sum(np.abs(fres)))
# print residual
# assert np.isclose(np.sum(np.abs(fres)), 0,  atol=1e-5)
