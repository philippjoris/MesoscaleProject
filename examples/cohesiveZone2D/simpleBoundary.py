...
matChz = GooseFEM.ConstitutiveModels.Cohesive2d(
    Kn=np.ones([nelemChz, nipChz])*100.0,
    Kt=np.ones([nelemChz, nipChz])*100.0,
    delta0=np.ones([nelemChz, nipChz])*0.0005,
    deltafrac=np.ones([nelemChz, nipChz])*0.028,
...
for ilam, lam in enumerate(np.linspace(0.0, 1.0, ninc)):
    ...
    du.fill(0.0)
    # update displacement
    du[mesh.nodesTopEdge, 1] = (+0.003/ninc)
